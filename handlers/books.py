...
from utils.ai import generate_recommendation_by_book
...
async def start_input_prompt(clbck: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(PromptStore.book_name)

@router.message(PromptStore.book_name, F.text)
async def book_name_accepted(message: Message, state: FSMContext):
    ...
    await state.set_state(PromptStore.book_author)

@router.message(PromptStore.book_author, F.text)
...
async def get_book_recommendations(message: Message, state: FSMContext):
    ...
    res = await generate_recommendation_by_book(prompt)
    if not res:
